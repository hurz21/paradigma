...
Public Class LocalParameterFiles
    ...
    Public Shared Sub erzeugeVorgangsListenDatei(ByVal sql As String, ByVal vliste As DataTable, ByRef user As String)
        Dim outfile As String = ""
        'alt:String.Format("{0}\aktvorgangsListe.txt", CLstart.mycSimple.Paradigma_local_root)


        outfile = initP.getValue("Myglobalz.paradigmaCache") & user & "\aktvorgangsListe.txt"
        l("outfile" & outfile)
        Using fs As New IO.StreamWriter(outfile)
            fs.WriteLine(sql)
            fs.WriteLine(sql)
            fs.WriteLine(sql)
            fs.WriteLine(sql)
