...
Public Class clsDBspecMSSQL
    ...
    Public Function manip(querie As String,
                        slqparamlist As List(Of LibDB.LIBDB.clsSqlparam),
                        ReturnIdentity As Boolean) As Integer Implements IDB_grundfunktionen.manipquerie
        ...
        Try
            ...
            LibDB.LIBDB.clsSqlparam.korrigiereParam(mydb.dbtyp, slqparamlist)
            ...
            If ReturnIdentity Then
                DBDT = New DataTable
                DBDA = New SqlDataAdapter(com)
                'anzahlTreffer& = DBDA.Fill(DBDT)
                newid = CLng(com.ExecuteScalar())
                'getNewID(anzahlTreffer, newid, com, DBDA, DBDT, myconn)
                myconn.Close()
                If newid < 1 Then
                    nachricht_und_Mbox("Problem beim Abspeichern : " & querie)
                    Return -1
                Else
                    Return CInt(newid)
                End If
    ...
    'Function getNewID(ByRef anzahlTreffer As Long, ByRef newid As Long, ByRef com As SqlCommand,
    '                      ByRef DBDA As SqlDataAdapter, ByRef DBDT As DataTable, MeineDBConnection As SqlConnection) As Long
    ...
    '    Try
    '        l("getNewID---------------------- anfang")
    '        com = New SqlCommand(ReturnQuery, MeineDBConnection)
    '        DBDT = New DataTable
    '        DBDA = New SqlDataAdapter(com)
    '        anzahlTreffer& = DBDA.Fill(DBDT)
    '        '-------------------------------
    '        newid = CLng(DBDT.Rows(0).Item(0))
    '        'newid = CLng(r("LastID"))
    '        'MsgBox(r("LastID").ToString)
    '        Return (newid)
    '        l("getNewID---------------------- ende")
    '    Catch ex As Exception
    '        l("Fehler in getNewID sqls: " & ex.ToString())
    '        Return -1
    '    End Try
    'End Function
