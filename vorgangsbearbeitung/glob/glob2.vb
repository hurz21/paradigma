...
Public Class glob2
    ...
    Shared Sub editVorgang(ByVal vorgangsnr As Integer, testmode As Boolean)

        glob2.holeDetailform(vorgangsnr, testmode)
    ...
    Public Shared Function EDIT_VorgangStamm_2DBOk() As Boolean
        ...
        If Not STammdatenInordnung(myGlobalz.sitzung.aktVorgang.Stammdaten) Then
    ...
    Public Shared Function NEU_VorgangStamm_2DBOk() As Boolean
        If Not STammdatenInordnung(myGlobalz.sitzung.aktVorgang.Stammdaten) Then
            ...
            Exit Function
        End If
        'myGlobalz.sitzung.aktVorgang.Stammdaten.hauptBearbeiter.ID = myGlobalz.sitzung.aktVorgang.Stammdaten.hauptBearbeiter.Initiale
        Debug.Print(myGlobalz.sitzung.aktVorgang.Stammdaten.hauptBearbeiter.Initiale)
        ...
        If erfolg Then
            ...
            If myGlobalz.sitzung.aktVorgang.Stammdaten.hauptBearbeiter.ID = 0 Then
                l("fehler NEU_VorgangStamm_2DBOk (main) exe hauptBearbeiter.ID = 0 ")
                'If 
    ...
    '    CLstart.VIDuebergabe.erzeugeParameterDatei(vorgangsnr,userid, CLstart.mycSimple.Paradigma_local_root)
    '    Dim modul As String = "O:\UMWELT -PARADIGMA\div\deployxcdetail\bin\debug\paradigma.exe "
    '    Process.Start(modul)
    'End Sub
    Public Shared Sub holeDetailform(ByVal vorgangsnr As Integer, testmode As Boolean)
        GC.Collect()
    ...
    Shared Function getMailuserid() As String
        Dim mailuserid$ = Microsoft.VisualBasic.Interaction.InputBox(
        "Bitte geben Sie hier Ihre Notes / Outlook - Userid ein:" & vbCrLf &
    ...
    Shared Function getMailpassword() As String
        Dim mailpw$ = Microsoft.VisualBasic.Interaction.InputBox(
        "Bitte geben Sie hier Ihr Notes / Outlook - Passwort ein:" & vbCrLf &
    ...
    Public Shared Function neueKarteerstellen(user As String) As String
        ...
        'End If
        Dim aufruf As String = generateKartenListenaufruf(user).Trim
        If Not aufruf.IsNothingOrEmpty Then
            'glob2.webmapBrows(aufruf)
            'Process.Start(aufruf)
            Dim result As String = CLstart.meineHttpNet.sendjobExtracted(aufruf, CLstart.mycSimple.enc, 18000000)
            ' Dim result As String = CLstart.meineHttpNet.meinHttpJob(myGlobalz.ProxyString, aufruf)
            '  MsgBox(result)
            Return result

        End If
        Return "aufruf ist leer: " & aufruf
    End Function

    Private Shared Function generateKartenListenaufruf(user As String) As String
        nachricht("generateKartenListenaufruf---------------------------------------------")
        Try
            Dim rumpf As String
            rumpf = initP.getValue("GisServer.URLlayer2shpfile")
            'rumpf &= myGlobalz.sitzung.aktBearbeiter.Initiale
            ...
            Return rumpf
        Catch ex As Exception
            nachricht("fehler in: generateKartenListenaufruf: " & ex.ToString)
            Return ""
        End Try
    End Function
    ...
    Private Shared Function STammdatenInordnung(mystamm As Stamm) As Boolean
        If String.IsNullOrEmpty(mystamm.az.gesamt) Then Return False
        If String.IsNullOrEmpty(mystamm.az.sachgebiet.Zahl) Then Return False
        If String.IsNullOrEmpty(mystamm.az.sachgebiet.Header) Then Return False
        Return True
    End Function
