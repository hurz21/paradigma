...
Public Class detailsTools
    ...
    Public Shared Sub EreignisExcel_ClickExtracted()
        Dim handcsv As New LIBcsvAusgabe.clsCSVausgaben("Ereignisse", myGlobalz.sitzung.EreignisseRec.dt, myGlobalz.sitzung.aktVorgangsID, "", CLstart.mycSimple.Paradigma_local_root, CLstart.mycSimple.enc)
    ...
    Public Shared Sub BeteiligteExcel_ClickExtracted()
        Dim handcsv As New LIBcsvAusgabe.clsCSVausgaben("Beteiligte", myGlobalz.sitzung.VorgangREC.dt, myGlobalz.sitzung.aktVorgangsID, "", CLstart.mycSimple.Paradigma_local_root, CLstart.mycSimple.enc)
        nachricht(" exportfile:" & handcsv.CscDateiAusgeben())
        handcsv.start()
        handcsv.Dispose()
    End Sub
    ...
    Public Shared Sub DokumenteExcel_clickExtracted()
        Dim handcsv As New LIBcsvAusgabe.clsCSVausgaben("Dokumente", myGlobalz.Arc.vorgangDocDt, myGlobalz.sitzung.aktVorgangsID, "", CLstart.mycSimple.Paradigma_local_root, CLstart.mycSimple.enc)
        nachricht(" exportfile:" & handcsv.CscDateiAusgeben())
        handcsv.start()
        handcsv.Dispose()
    End Sub
    ...
    'Private Shared Function GetMesresDOK(ByVal dok As Dokument) As MessageBoxResult
    ...
    'Shared Function wurdenDokumenteGeaendert(ByVal checkoutDokuList As List(Of Dokument)) As Boolean
    ...
    'Private Shared Function KopiereZu(ByVal CO_test As IO.FileInfo, ByVal dok As Dokument) As Boolean
    '    Try
    ...
    'Private Shared Sub geaenderteDateiInsArchivUebernehmen(ByVal CO_test As IO.FileInfo, ByVal dok As Dokument)
    ...
    '            myglobalz.sitzung.aktDokument.makeFullname_Checkout(quellVorgangsID, myglobalz.Arc.lokalerCheckoutcache, 0)
    ...
    Shared Function erstelleCSVausgabeDerFlurstuecke(ByVal p1 As Integer) As Boolean
        Dim handcsv As New LIBcsvAusgabe.clsCSVausgaben("FlurstueckeFuerEigentuemer", myGlobalz.sitzung.raumbezugsRec.dt, myGlobalz.sitzung.aktVorgangsID, "", CLstart.mycSimple.Paradigma_local_root, CLstart.mycSimple.enc)
        nachricht(" exportfile:" & handcsv.CscDateiAusgeben())
        handcsv.Dispose()
    End Function
    ...
    '    Try
    '        myGlobalz.sitzung.EreignisseRec.mydb.SQL = "select * from  " & CLstart.myViewsNTabs.tabEreignis & "  where datum=(select max(datum)   from  " & CLstart.myViewsNTabs.tabEreignis & "   where vorgangsid=" &
    '             myGlobalz.sitzung.aktVorgangsID & ")"
    ...
    '    Dim neutext As String = "", hinweis As String = ""
    '    myGlobalz.sitzung.EreignisseRec.mydb.SQL = "select * from " & CLstart.myViewsNTabs.tabStammdaten &
    '                 " where vorgangsid in " &
    ...
    Public Shared Sub MapfileTemplateBearbeiten(ByVal AusgabeRootDir As String)
        ...
        mapgeneratortools.makeMapFile(KartenMapfileTemplate, kartenmapfile, myglobalz.sitzung.aktBearbeiter.username, True, CLstart.mycSimple.enc,
