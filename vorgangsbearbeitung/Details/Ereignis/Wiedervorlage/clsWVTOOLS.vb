...
Public Class clsWVTOOLS
    ...
    Shared Function WVrecord2OBJ(ByRef wvl As clsWiedervorlage, ByRef REC As DataTable) As Boolean  'myGlobalz.sitzung.aktWiedervorlage		'REC
        Try
            ...
            wvl.gemkrz = CStr(REC.Rows(0).Item("gemkrz"))
            wvl.az2 = CStr(REC.Rows(0).Item("az2"))
            Return True
        Catch ex As Exception
        ...
        End Try
    End Function
    Private Property selectString As String = "SELECT w.VorgangsID,w.Datum,w.WartenAuf,w.Bemerkung , " &
                                                     "w.Erledigtam,w.Erledigt,w.ToDo,w.Bearbeiter,v.az,s.Beschreibung,s.az2,s.gemkrz ,s.BEARBEITERid "
    ...
    Public Sub getWiedervorlageDT(ByVal Bearbeiterid As String)
        Dim bearbeiterString As String = ""
        Try
            selectString = selectString.ToUpper
            If String.IsNullOrEmpty(Bearbeiterid.Trim) OrElse Bearbeiterid.Trim.ToLower = "alle" Then
                bearbeiterString = ""
            Else
                bearbeiterString = String.Format(" and w.BEARBEITERid={0} ", Bearbeiterid.ToLower)
            End If
