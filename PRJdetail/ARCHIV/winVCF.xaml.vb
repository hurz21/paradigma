#Disable Warning BC40056 ' Namespace or type specified in the Imports 'jfcardreader.vCard' doesn't contain any public member or cannot be found. Make sure the namespace or the type is defined and contains at least one public member. Make sure the imported element name doesn't use any aliases.
Imports jfcardreader.vCard
#Enable Warning BC40056 ' Namespace or type specified in the Imports 'jfcardreader.vCard' doesn't contain any public member or cannot be found. Make sure the namespace or the type is defined and contains at least one public member. Make sure the imported element name doesn't use any aliases.
Public Class winVCF

    Private Property imail As String
    Private _adresses As List(Of jfcardreader.vCard.Address)
    Private _jfread As jfcardreader.vCard.vCardReader
    Private _phones As List(Of jfcardreader.vCard.Phone)
    Private _DateinameFull As String
     Private Sub gastLayout()
        Background = myGlobalz.GetSecondBackground()
        grdMain.Background = myGlobalz.GetSecondBackground()        
    End Sub
    ...
    Sub vcardread(ByVal datei As String)
        _jfread = New jfcardreader.vCard.vCardReader()
        Dim vstring As String
        Dim lines As String()
        Dim test As String = ""
        _phones = New List(Of jfcardreader.vCard.Phone)()
        _adresses = New List(Of jfcardreader.vCard.Address)()
    ...
    Private Sub phonesSetzen(ByVal phones As List(Of jfcardreader.vCard.Phone))
        Try
            tbTel1.Text = phones(0).number
            tbFax1.Text = phones(0).number
            tbMobil1.Text = phones(0).number
            For Each pho In phones
                If pho.homeWorkType = jfcardreader.vCard.HomeWorkType.home Then
                    If pho.phoneType = jfcardreader.vCard.PhoneType.VOICE Then
                        tbTel1.Text = pho.number
                    End If
                    If pho.phoneType = jfcardreader.vCard.PhoneType.FAX Then
                        tbFax1.Text = pho.number
                    End If
                    If pho.phoneType = jfcardreader.vCard.PhoneType.CELL Then
                        tbMobil1.Text = pho.number
                    End If
                End If
                If pho.homeWorkType = jfcardreader.vCard.HomeWorkType.work Then
                    If pho.phoneType = jfcardreader.vCard.PhoneType.VOICE Then
                        tbTel1.Text = pho.number
                    End If
                    If pho.phoneType = jfcardreader.vCard.PhoneType.FAX Then
    ...
    Private Sub adressenSetzen(ByVal adresses As List(Of jfcardreader.vCard.Address))
        Try
    ...
    Public Sub PersonUbernehmen()
        Try
            ...
            End With
        Catch ex As Exception
            l("Fehler bei der Übernahme von Daten aus der Vorgangsdatenbank! in PersonAusVorgangsDBUebernehmen", ex)
        End Try
    End Sub
