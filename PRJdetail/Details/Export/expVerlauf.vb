...
Module expVerlauf
    Sub VERLAUF_CsvListeErstellen(datei As String)
        Dim handcsv As New LIBcsvAusgabe.clsCSVausgaben("Ereignisse", myGlobalz.sitzung.EreignisseRec.dt,
    ...
    Private Function GetDateinameVerlauf(ByVal pfad_verlauf As String, ByVal zeile As DataRow, endung As String) As String
        ...
        dateiname = dateiname &  LIBgemeinsames.clsString.date2string(CDate(zeile.Item("DATUM")), 2).trim
        dateiname = dateiname & trennerInFileNames & LIBgemeinsames.clsString.normalize_Filename(zeile.Item("ART").ToString).Trim
        dateiname = dateiname & trennerInFileNames & LIBgemeinsames.clsString.normalize_Filename(LIBgemeinsames.clsString.kuerzeTextauf(zeile.Item("BESCHREIBUNG").ToString, 50)).Trim
    ...
    Private Function getBRforCR(txt As String) As String
        ...
        Catch ex As Exception
            nachricht("fehler in getBRforCR: " ,ex)
            Return ""
        End Try
    End Function
