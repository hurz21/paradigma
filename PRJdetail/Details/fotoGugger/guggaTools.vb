...
Module guggaTools
    ...
    Public Sub kopiereDokMetadaten(ByVal zieldok As Dokument, ByVal quelldok As Dokument)
        With quelldok
            ...
            If zieldok.ExifDatum < CDate("1970-01-01") Then zieldok.ExifDatum = zieldok.Filedatum
            ...
            zieldok.DocID = .DocID
            zieldok.newSaveMode = .newSaveMode
        End With
    End Sub
    ...
    Public Sub initSetEtikett(ByVal collFotos As List(Of clsFotoDokument))
        ...
        For Each adok As clsFotoDokument In collFotos
            If adok.ExifDatum < CDate("1970-01-01") Then
                tempdatum = adok.Filedatum
            Else
                tempdatum = adok.ExifDatum
            End If
            adok.Etikett = adok.Beschreibung & Environment.NewLine &
                tempdatum & Environment.NewLine &
    ...
    Private Function koordinatenloeschen(ByVal aktfoto As Dokument) As Boolean
        ...
        Try
            ...
            Return True
        Catch ex As Exception
            nachricht("fehler in koordinatenloeschen: " ,ex)
            Return False
        End Try
    End Function
    ...
    Friend Function fotoloeschen(newdok As clsPresDokumente, mitnachfrage As Boolean) As Boolean
        ...
        If detailsTools.dokumentIstGeoeffnet(newdok) Then
            MessageBox.Show("Das Dokument ist noch geöffnet. " & Environment.NewLine &
                                ...
                                newdok.DateinameMitExtension)
            Return False
        End If
        If mitnachfrage Then
            If glob2.istloeschenErnstgemeint() Then
                DokArc.aktDokumentLoschen(newdok)
                CLstart.myc.aLog.komponente = "Dokumente" : CLstart.myc.aLog.aktion = String.Format("{0} {1}: geloescht", newdok.DocID, newdok.DateinameMitExtension) : CLstart.myc.aLog.log()
                Return True
            End If
        Else
        ...
        'If glob2.istloeschenErnstgemeint() Then
        '    DokArc.aktDokumentLoschen(newdok)
        '    'DokArc.ausCheckoutlisteEntfernen(newdok, myglobalz.sitzung.checkoutDokuList)
        '    CLstart.myc.aLog.komponente = "Dokumente" : CLstart.myc.aLog.aktion = String.Format("{0} {1}: geloescht", newdok.DocID, newdok.DateinameMitExtension) : CLstart.myc.aLog.log()
        '    Return True
        'End If
        Return False
