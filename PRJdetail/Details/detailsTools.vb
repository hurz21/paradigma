...
#Enable Warning BC42358 ' Because this call is not awaited, execution of the current method continues before the call is completed. Consider applying the Await operator to the result of the call.
    ...
    Public Shared Function Archiv_aktiviere_Dokument(dokumenteRitemousekeypressed As Boolean, readOnlyDoxsInTxtCrtlOeffnen As Boolean, aktThumbnailNotiz As String,
        ...
        If box.knopfnummer = 1 Then
            ...
            If DokArc.machCheckout(lfehler) Then
                ...
                DokArc.Archiv_aktiviere_dokument(myGlobalz.sitzung.aktDokument, readOnlyDoxsInTxtCrtlOeffnen, alleBilder, eid)
        ...
        If box.knopfnummer = 14 Then
            ...
            Else
                If DokArc.machCheckout(lfehler) Then
                    DokArc.Archiv_aktiviere_dokument(myGlobalz.sitzung.aktDokument, CBool(box.readOnlyDoxsInTxtCrtlOeffnenLOKAL), alleBilder, eid)
                Else
                    box.dokumentWurdeGeoeffnet = False
                    Return False
                End If
    ...
    End Function
    Shared Sub starteProbaugView()
        Dim jahr, nummer As String
        Dim erfolg As Boolean
        Dim si As New ProcessStartInfo
        Try
            l(" MOD starteProbaugView anfang")
            ...
            If erfolg Then
                si.FileName = "C:\kreisoffenbach\probaugView\probaugView.exe "
                si.WorkingDirectory = "C:\kreisoffenbach\probaugView"
                si.Arguments = " #/vid=" & myGlobalz.sitzung.aktVorgangsID & " /jahr=" & jahr & " /nummer=" & nummer &
                    " /bid=" & myGlobalz.sitzung.aktBearbeiter.ID &
                    " /bname=" & myGlobalz.sitzung.aktBearbeiter.username &
                    " /initial=" & myGlobalz.sitzung.aktBearbeiter.Initiale
                Process.Start(si)
                si = Nothing
            End If
            l(" MOD starteProbaugView ende")
        Catch ex As Exception
            l("Fehler in starteProbaugView: " & ex.ToString())
        End Try
        l(" MOD starteProbaugView ende")
    End Sub

    Friend Shared Function zerlegeProbaugAZ(probaugaz As String, ByRef jahr As String, ByRef nummer As String) As Boolean
        Dim tempj, tempn, feld() As String
        ' 63-00690-10-24-Va 
        Try
            l(" MOD zerlegeProbaugAZ anfang")
            If Not probaugaz.Contains("-") Then Return False
            feld = probaugaz.Trim.Split("-"c)
            If feld(0).Trim = "63" Then
                nummer = feld(1).Trim
                jahr = feld(2).Trim
                Return True
            Else
                Return False
            End If
            l(" MOD zerlegeProbaugAZ ende")
            Return True
        Catch ex As Exception
            l("Fehler in zerlegeProbaugAZ: " & ex.ToString())
            Return False
        End Try
    End Function
        ...
        Try
            For Each dok As DataRow In dokumenteDatatable.Rows
                If CInt(clsDBtools.fieldvalue(dok.Item("ID"))) <> aktereig.ID Then
                    ...
                    Continue For
                End If
                aktereig = New clsEreignisDok With {
                    .Notiz = "",
                    .Beschreibung = clsDBtools.fieldvalue(dok.Item("DATEINAMEEXT")),
                    .ID = CInt(clsDBtools.fieldvalue(dok.Item("ID"))),
                    .DokumentID = CInt(clsDBtools.fieldvalue(dok.Item("DOKUMENTID"))),
                    .Quelle = clsDBtools.fieldvalue(dok.Item("QUELLE")),
                    .typnr = CInt(clsDBtools.fieldvalue(dok.Item("TYPNR"))),
                    .Datum = CDate(clsDBtools.fieldvalueDate(dok.Item("DATUM")))
                }
                aktereig.dok.Filedatum = CDate(clsDBtools.fieldvalue(dok.Item("DATUM")))
                Try
    ...
    Public Shared Sub Dokument2Obj(item As clsEreignisDok, dokument As Dokument)
        Try
            ...
            Try
                Debug.Print((clsDBtools.toBool(item.dok.istNurVerwandt)) & "")
                dokument.istNurVerwandt = (clsDBtools.toBool(item.dok.istNurVerwandt)) 'True 'CBool(clsDBtools.toBool(item.dok.status)
            Catch ex As Exception
            End Try
        Catch ex As Exception
