...
Public Class clsMiniMapTools
    ...
    Public Shared Function holePasswordFuerGisUser_dballe(ByVal username As string, ByVal rec As IDB_grundfunktionen) As String 'myGlobalz.Bearbeiter
        ...
        If myGlobalz.webgis_MYDB.dbtyp = "mysql" Then
            ...
            pw$ = wbc.holePasswordFuerGisUser(username, rec) 'myGlobalz.Bearbeiter, myGlobalz.haloREC)
        End If
    ...
    Public Shared Function getDT4anySQL_ALLEDB_dballe(ByVal sql As String, ByVal rec As IDB_grundfunktionen) As DataTable
        ...
        If myGlobalz.webgis_MYDB.dbtyp = "mysql" Then
            Dim wbc As New clsWebgiscontrol_Mysql(clsDBspecMYSQL.getConnection(myGlobalz.webgis_MYDB))
            datatabelle = wbc.getDT4anySQL(sql, rec) 'myGlobalz.Bearbeiter, myGlobalz.haloREC)
        End If
        If myGlobalz.webgis_MYDB.dbtyp = "oracle" Then
            Dim wbc As New clsWebgiscontrol_Oracle(clsDBspecMYSQL.getConnection(myGlobalz.webgis_MYDB))
            datatabelle = wbc.getDT4anySQL(sql, rec) 'myGlobalz.Bearbeiter, myGlobalz.haloREC)
    ...
    Private Shared Sub getMapshareUser()
        ...
        If initP.getValue("MiniMap.MapStatus") = "mapshare" Then
            pwtet = holePasswordFuerGisUser_dballe(myGlobalz.sitzung.aktBearbeiter.username, myGlobalz.sitzung.haloREC)
    ...
    Sub defineRange(ByVal xl As Double, ByVal xh As Double, ByVal yl As Double, ByVal yh As Double, ByVal zielrange As LibGISmapgenerator.clsRange) 'clstart.myc.kartengen.aktrange
        zielrange.xl = xl
        zielrange.xh = xh
        zielrange.yl = yl
        zielrange.yh = yh
    ...
    Shared Sub ausschnittNeuBerechnen(ByVal RubberbandStartpt As Point?, ByVal RubberbandEndpt As Point?)
        Try
            ...
            newpoint = LibGISmapgenerator.clsAufrufgenerator.WINPOINTVonCanvasNachGKumrechnen(newpoint, clstart.myc.kartengen.aktMap.aktrange, clstart.myc.kartengen.aktMap.aktcanvas)
            newpoint2 = LibGISmapgenerator.clsAufrufgenerator.WINPOINTVonCanvasNachGKumrechnen(newpoint2, clstart.myc.kartengen.aktMap.aktrange, clstart.myc.kartengen.aktMap.aktcanvas)
    ...
    Public Shared Function MyPointVonCanvasNachGKumrechnen(ByVal ptA As Point) As myPoint
        ...
        ptTemp.X = ptA.X
        ptTemp.Y = ptA.Y
        ptTemp2 = LibGISmapgenerator.clsAufrufgenerator.WINPOINTVonCanvasNachGKumrechnen(ptTemp, clstart.myc.kartengen.aktMap.aktrange, clstart.myc.kartengen.aktMap.aktcanvas)
        ptA.X = ptTemp2.X
        ptA.Y = ptTemp2.Y
        Return ptTemp2
    End Function
    ...
    Shared Function setMapCookie(ByVal aktmap As clsMapSpec, ByVal vid As Integer) As Boolean
        Dim mcookie As New DB_Minimap_ORACLE(LIBoracle.clsDBspecOracle.getConnection(myGlobalz.raumbezug_MYDB))
    ...
    Public Shared Sub setzeAbteilungsThemen(ByVal aktmap As clsMapSpec, abteilung As String)
        Dim akttheme As String = "" 'myGlobalz.sitzung.VorgangsID & ";"
        Select Case abteilung 'myGlobalz.sitzung.aktVorgang.Stammdaten.Bearbeiter.Bemerkung.Trim
            Case "Immisionsschutz", "Immissionsschutz"
                aktmap.Vgrund = akttheme & "altpro09;altflaeche;kehrbezirk;bplanrechtsw;altis2012;gemarkung_"
                aktmap.Hgrund = "realshapeopak"
                aktmap.ActiveLayer = "altis2012"
            Case "Untere Naturschutzbehörde"
                aktmap.Vgrund = akttheme & "natlandgeb;lsgschilder;waldabteilung;kompensation;bplanrechtsw;gemarkung_"
                aktmap.Hgrund = "realshapeopak"
                aktmap.ActiveLayer = "natlandgeb"
            Case "Untere Wasserbehörde"
                aktmap.Vgrund = akttheme & "wsgeb;wsgeb3;oberfl;ueberschw;altis2012;gemarkung_;"
                aktmap.Hgrund = "tk5"
                aktmap.ActiveLayer = "altis2012;"
            Case "Schornsteinfegerwesen"
                aktmap.Vgrund = akttheme & "kehrbezirk;gemarkung_;"
                aktmap.Hgrund = "realshapeopak"
                aktmap.ActiveLayer = "kehrbezirk"
            Case "Graphische Datenverarbeitung"
                aktmap.Vgrund = akttheme & "gemarkung_;"
                aktmap.Hgrund = "tk5"
                aktmap.ActiveLayer = "tk5"
            Case "Assistenz"
                aktmap.Vgrund = akttheme & "kehrbezirk;bplanrechtsw;gemarkung_;"
                aktmap.Hgrund = "realshapeopak"
                aktmap.ActiveLayer = ""
            Case "Stv. Fachdienstleitung", "Fachdienstleitung"
                aktmap.Vgrund = akttheme & "natlandgeb;lsgschilder;waldabteilung;kompensation;gemarkung_;"
                aktmap.Hgrund = "realshapeopak"
                aktmap.ActiveLayer = "natlandgeb"
            Case Else
                aktmap.Vgrund = akttheme & "gemarkung_;"
                aktmap.Hgrund = "realshapeopak;"
                aktmap.ActiveLayer = "realshapeopak"
        End Select
    End Sub

    Shared Function killMapCookie(ByVal vid As Integer) As Boolean
        Dim mcookie As New DB_Minimap_ORACLE(LIBoracle.clsDBspecOracle.getConnection(myGlobalz.raumbezug_MYDB))
        Dim istvorhanden As Boolean = mcookie.killmapcookie(vid)
        mcookie.Dispose()
    End Function
    ...
    Public Shared Function dbabfrageTK5(ByVal original As String, zeitInMS As Integer) As String
        ...
        Try
            ...
            Dim antwort As String = glob2.sendjobExtracted(aufruf, CLstart.mycsimple.enc, zeitInMS)
    ...
    Public Shared Function bildeAufruf(ByVal objektid As String, aktive_ebene As String, templatefile As String) As String
        ...
        Try
            param = "?lookup=true"
            param = param & "&user=" + myGlobalz.sitzung.aktBearbeiter.username + "&passwort=" & clsMiniMapTools.holePasswordFuerGisUser_dballe(myGlobalz.sitzung.aktBearbeiter.username, myGlobalz.sitzung.haloREC)
    ...
    Private Shared Sub oeffneConnectionEigentuemer(ByRef EigentuemerSchnellDB As OracleConnection)
        Try
   ...
   ";")
        ...
        End Try
    End Sub
    Private Shared Function getEigentuemerdata(ByVal EigentuemerSchnellDB As OracleConnection, ByVal fs As String,
                                               ByRef kurzinfo As String, nameundadresse As String,
                                    ByRef _mycount As Integer,
                                    ByRef dt As DataTable) As OracleCommand
    ...
    Private Shared Function convertImagamap2Eigentuermap(ByRef Mapdatei As String) As String
        Dim line As String
        Try
            Dim test As New IO.FileInfo(Mapdatei)
            ...
            neuedatei = Mapdatei.Replace(".txt", ".eig")
            Dim sb As New System.Text.StringBuilder
            Dim eigentschnellKonn As New Devart.Common.DbConnectionStringBuilder
            Dim EigentuemerSchnellDB As OracleConnection = New OracleConnection()
            oeffneConnectionEigentuemer(EigentuemerSchnellDB)
            Dim com As OracleCommand = EigentuemerSchnellDB.CreateCommand()
            Using sr As New IO.StreamReader(Mapdatei, CLstart.mycsimple.enc)
                Dim fs As String
                Dim nameundadresse As String = ""
                Dim kurzinfo As String = ""
                Dim coords, shape, href, title As String
                ...
                Do
                    ...
                    title = GetTagValue(line, "title=")
                    href = GetTagValue(line, "href=")
                    fs = getFSfromHref(href)
                    ' fs = "06075700400238______"
                    If Not fs.IsNothingOrEmpty Then
                        com = getEigentuemerdata(EigentuemerSchnellDB, fs, kurzinfo, nameundadresse, _mycount, dt)
                        ' erfolg = getEigenuemerFromDB(fs, nameundadresse, kurzinfo)
                        title = kurzinfo
                    End If
                    ...
                    myPointCollection = bildeMyPointCollection(coords)
                    If Not IsNothing(myPointCollection) Then
                        NeuesImageFileKonstruieren(sb, " shape=poly ", title, coords, href)
                    End If
                Loop
                eigentuemerdbSchliessen(EigentuemerSchnellDB)
            End Using
            My.Computer.FileSystem.WriteAllText(neuedatei, sb.ToString, False)
            Return neuedatei
        Catch ex As Exception
            nachricht("Fehler in imageMap2PolygonMap: " & ex.ToString)
            Return Mapdatei
    ...
    Private Shared Sub eigentuemerdbSchliessen(EigentuemerSchnellDB As OracleConnection)
    ...
    Private Shared Function bildeNASAufruf(aktflst As clsFlurstueck, aktive_ebene As String, templatefile As String) As String
        ...
        Try
            ...
            param = param & "&username=" + myGlobalz.sitzung.aktBearbeiter.username + "&password=" &
                clsMiniMapTools.holePasswordFuerGisUser_dballe(myGlobalz.sitzung.aktBearbeiter.username, myGlobalz.sitzung.haloREC)
            param = param & "&aktive_ebene=" + aktive_ebene
            'param = param & "&object_id=" + objektid
            'param = param & "&templatefile=" + templatefile
            param = param & "&activelayer=" + aktive_ebene
            ...
            '      Param="/cgi-bin/apps/gis/albnas2/albnas2.cgi"
            '+ "?username=" + top.me.name
            '+ "&password=" + top.me.pw
            '+ "&gemcode=" + '[FELDINHALT2]'
            '+ "&flur=" + '[FELDINHALT3]'
            '+ "&fstueck=" + '[FELDINHALT4]'
    ...
    Private Shared Function getFS4sekid(sekid As Integer) As String
        Try
            myGlobalz.sitzung.tempREC2.mydb.SQL = "select fs from paraflurstueck where id=" & sekid
