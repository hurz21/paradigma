...
Public Class FST_tools
    ...
    Public Shared Function RBFlstNeu_alleDB() As Integer
        ...
        If myGlobalz.raumbezug_MYDB.dbtyp = "oracle" Then
            Dim zzz As New FST_DBTool_Oracle(LIBoracle.clsDBspecOracle.getConnection(myGlobalz.raumbezug_MYDB))
            result% = zzz.RB_FLST_abspeichern_Neu()
            zzz.Dispose()
        End If
        Return result
    ...
    Public Shared Sub hole_FSTKoordinaten_undZuweisePunkt(ByVal afst As ParaFlurstueck)
        Try
            ...
            If Not myGlobalz.sitzung.fstREC.dt.IsNothingOrEmpty Then
                BASIS_vonDTnachObjekt(afst, myGlobalz.sitzung.fstREC.dt)
                myGlobalz.sitzung.aktFST.FlaecheQm = CDbl((myGlobalz.sitzung.fstREC.dt.Rows(0).Item("gisarea")))
    ...
    Shared Function FSTSchonInVorgangVorhanden(ByVal paraFlurstueck As ParaFlurstueck, ByVal vid As Integer) As Boolean
        myGlobalz.sitzung.tempREC.mydb.SQL =
              "select * from paraflurstueck p , pf_sekid2vid s where gemcode=" & paraFlurstueck.normflst.gemcode &
