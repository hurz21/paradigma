...
Public Class clsAdress
    ...
    Public Function clean(plz As String) As String
        ...
        If String.IsNullOrEmpty(plz) Then
            adresse = String.Format("{0}, {1} {2}, Deutschland",
                                                 clsString.normalizeSuchdbStrasse(gemeindeName),
                                                 clsString.normalizeSuchdbStrasse(strasseName),
                                                 HausKombi)
        Else
            adresse = String.Format("{0}, {1} {2}, Deutschland",
                                                 plz.Trim,
    ...
    '                                      ByVal tempzusatz as string) as  Boolean
...
zum_ende_der_hnr:
            ...
            ''For i = anfang_nr + 1 To Len(_strasseName)
            ''  If Not IsNumeric(Mid$(_strasseName, i, 1)) Then
            ''    'mylog "DEBUCK strassehausnummer_zerlegen: ende" & i
            ''    If anfang_nr > 0 Then
            ''      ende_nr = i
            ''      GoTo fertig
            ''    End If
...
fertig:
            'mylog "DEBUCK strassehausnummer_zerlegen: ende_nr " & ende_nr
            strasseName = Mid$(_strasseName, 1, anfang_nr - 1).Trim.ToLower
            'mylog "DEBUCK strassehausnummer_zerlegen: tempstrasse$ " & tempstrasse$

            'temphausnummer% = Val(Mid$(suchstrasse, anfang_nr, Len(suchstrasse) - ende_nr + 1))
            '_hausNr = CInt(Val(Mid$(_strasseName, anfang_nr, Len(_strasseName) - anfang_nr + 1)))
            _HausKombi = Mid$(lStrasseName, anfang_nr, Len(lStrasseName) - anfang_nr + 1)

            'mylog "DEBUCK strassehausnummer_zerlegen: temphausnummer% " & temphausnummer% & _
            '(Len(suchstrasse) - anfang_nr + 1), _
            'suchstrasse, CStr(anfang_nr)

            '_hausZusatz = Mid$(_strasseName, anfang_nr, Len(_strasseName) - anfang_nr + 1)

            '_HausKombi = _hausNr & _hausZusatz
            'mylog "DEBUCK strassehausnummer_zerlegen: tempzusatz$: " & tempzusatz$
            'mylog "DEBUCK strassehausnummer_zerlegen: ############################################## Ausgang"

        Catch ex As Exception
            nachricht("Fehler in is_strasse_mit_hausnummer " & ex.ToString)
        End Try
    End Function
    Public Sub hauskombiZerlegen()
        ...
        Catch ex As Exception
            Dim FehlerHinweis$ = "Error / Fehler: " & vbCrLf + _
             ex.Message + " " & vbCrLf + _
