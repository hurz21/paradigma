Public Class SQL_Beteiligte
    ...
    Public Sub compoze()
        ...
        Try
            ...
            Dim orderstring$ = " order by name,vorname,gemeindename,strasse "
            result = anfang & sqlname & sqlvorname & sqlstadt$ & sqlstrasse & orderstring$ & limitstring
            glob2.nachricht(String.Format("1compoze: {0}{1}", vbCrLf, result))
    ...
    Private Function GetSqlname$()
        ...
        If String.IsNullOrEmpty(name) Then
            sqlname = ""
        Else
            sqlname = String.Format(" and lower(b.name) like '{0}%'", name$.ToLower)
        End If
        Return sqlname
    End Function
    ...
    Private Shared Function GetAnfang$()
        Dim anfang$ = "select distinct  s.VorgangsID, s.Ortstermin,s.Stellungnahme, v.AZ, s.Beschreibung," + _
                 " s.LetzteBearbeitung,s.LastActionHeroe, s.Eingang ,b.Name ,b.Vorname,b.Gemeindename, b.Strasse" & _
