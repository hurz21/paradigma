...
Partial Public Class Window_Person
    Public lastpersonDB As String
    Private Property _sql As String
    Property _namensFilter As String
    Private Sub gastLayout()
        Background = myGlobalz.GetSecondBackground()
        stckMain.Background = myGlobalz.GetSecondBackground()
    End Sub
    ...
    Private Sub Window_Person_Closing(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Me.Closing
        ...
        End If
    End Sub
    Sub New(ByVal sql As String, namensFilter As String)
        InitializeComponent()
        _sql = sql
        _namensFilter = namensFilter.Trim.ToLower
    End Sub
    Private Sub Window_Person_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        ...
        Title = detailsTools.settitle("Personenauswahl" & "")

        If Not String.IsNullOrEmpty(_sql) Then 'rollensuche
            tbFilter.Text = _sql
            suchePerson(tbFilter.Text, "Vorgangs-DBGesellfunktion", tbFilterOrt.Text)
        Else
            If Not _namensFilter.IsNothingOrEmpty() Then
                tbFilter.Text = _namensFilter
                cmbPersonalDB.SelectedValue = "Vorgangs-DB"
                suchePerson(tbFilter.Text, "Vorgangs-DB", tbFilterOrt.Text)
    ...
    Private Sub suchePerson(ByVal filter As String, ByVal personalDBname$, ByVal filterort$)
        ...
        Catch ex As Exception
            l("Fehler suchePerson Keine Treffer:	", ex)
        Finally
    ...
    Private Sub dgPersonenvorschlaege_SelectionChanged(ByVal sender As System.Object, ByVal e As System.Windows.Controls.SelectionChangedEventArgs) Handles dgPersonenvorschlaege.SelectionChanged
        personvorschlagen()
        DialogResult = True
        e.Handled = True
        Me.Close()
    ...
    Private Function PersonUebernehmen(ByVal name As String, ByVal vorname As String, ByVal item As DataRowView) As Boolean
        Try
    ...
    Private Shared Sub PersonAusProbaugNachParadigmadbUEbernehmen(ByVal item As DataRowView)
        ...
        Try
            myGlobalz.sitzung.aktPerson.Kassenkonto = CStr(clsDBtools.fieldvalue(item("KASSENKONTO"))).ToString()
        Catch ex As Exception
    ...
    Function personExistiertInParadigmaDB(ByVal name$, ByVal vorname As String) As Boolean
        Dim sqltext = ""
        Dim vornameSQLstring As String = ""
        Try
            If String.IsNullOrEmpty(name) Then
                nachricht_und_Mbox("Warnung: Nachname ist leer. Abbruch")
            End If

            If String.IsNullOrEmpty(vorname) Then
                vornameSQLstring = " and upper(Vorname)= '" & vorname.ToUpper & "'"
            End If
            myGlobalz.sitzung.tempREC.mydb.Tabelle = myGlobalz.vorgangsbeteiligte_MYDB.Tabelle
            sqltext = "SELECT * FROM " & myGlobalz.vorgangsbeteiligte_MYDB.Tabelle &
             " where upper(NACHNAME) = '" & name.ToUpper & "'" &
            vornameSQLstring
    ...
    Sub personvorschlagen()
        ...
        Catch ex As Exception
            nachricht("fehler in dgEreignisse_SelectionChanged: ", ex)
        End Try
    End Sub
