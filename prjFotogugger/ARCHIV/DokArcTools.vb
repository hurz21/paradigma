...
Namespace DokArcTools
    ...
    Public Class dokusVonEreignisHolen
        Shared Function execute(ByVal EreignisID As Integer) As Boolean
            Try
                ...
                myglobalz.Arc.ArcRec.mydb.SQL = DokArcOracle.bildesqlStringFuerdokumente("beides", myglobalz.Arc.ArcRec.mydb.SQL)
    ...
    End Class
    Public Class divers
        Friend Shared Function getDokumentforDocid(givenDOCID As Integer, presf As List(Of clsPresDokumente)) As Dokument
            Try
                For Each doku As Dokument In presf
                    If givenDOCID = 0 Then
                        Return doku
                    End If
                    If doku.DocID = givenDOCID Then
                            Return doku
                        End If
                Next
            Catch ex As Exception
