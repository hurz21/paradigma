...
#End Region
    ...
    Public Shared Sub DokumentDatarowView2Obj(ByVal item As DataRowView, ByVal dokulok As Dokument)
        ...
        End Try
    End Sub
    Public Shared Sub JPG_handeln(ByVal aktdoku As Dokument, allebilder As Boolean, eid As Integer, vidlist As String)
        Try
            If DokArc.aktiviereFotoGucker(myglobalz.sitzung.aktVorgangsID, vidlist, myglobalz.OhneObsoletenDokus, eid, allebilder) > 0 Then
    ...
    Public Shared Function Archiv_aktiviere_dokument(ByVal aktdoku As Dokument, readOnlyDoxsInTxtCrtlOeffnen As Boolean, allebilder As Boolean, eid As Integer,
                                                     vidlist As String) As Boolean
        ...
        If Not String.IsNullOrEmpty(aktdoku.FullnameImArchiv) Then
            Return doktypOeffnen(aktdoku, readOnlyDoxsInTxtCrtlOeffnen, allebilder, eid, vidlist)
        Else
            nachricht_und_Mbox("Dateiname ist ungültig:" & aktdoku.FullnameImArchiv)
        End If
    End Function

    Public Shared Function doktypOeffnen(ByVal aktdoku As Dokument, readOnlyDoxsInTxtCrtlOeffnen As Boolean, allebilder As Boolean, eid As Integer,
                                         vidlist As String) As Boolean
        ...
        Select Case test.Extension.ToLower
            ...
            Case ".png", ".gif", ".jpg", ".jpeg"  'foto
                JPG_handeln(aktdoku, allebilder, eid, vidlist)
    ...
    Public Shared Sub ausCheckoutlisteEntfernen(ByVal dok As Dokument, ByRef dieliste As List(Of Dokument))
        ...
        End If
    End Sub
    'Shared Function aktiviereFotoGucker(ByVal vorgangsID As Integer, ByVal OhneObsoletenDoku As Boolean, eid As Integer, alleBilder As Boolean) As Integer
    ...
    'End Function
    Shared Function aktiviereFotoGucker(ByVal vorgangsID%, vidlist As String, ByVal OhneObsoletenDoku As Boolean, eid As Integer, alleBilder As Boolean) As Integer
        'alle Fotos in Objektliste packen
        Dim auchVeraltete = True

        Dim bresult As Boolean
        If vorgangsID < 1 And vidlist.Trim.Count > 1 Then
            bresult = DokArcTools.dokusVonVorgangHolen.execute(vidlist, "nurfotos", alleBilder, eid)
        Else
            bresult = DokArcTools.dokusVonVorgangHolen.execute(CStr(vorgangsID), "nurfotos", alleBilder, eid)
        End If
            ...
            For i = 0 To myGlobalz.Arc.ArcRec.dt.Rows.Count - 1
                If istFotoTyp(myGlobalz.Arc.ArcRec.dt.Rows(i).Item("Typ").ToString.ToLower) Then
                    ...
                    If Not dokulok.istVeraltet Or auchVeraltete Then
                        ...
                        Dim ausgabeVerzeichnis As String = ""
                        Debug.Print(myglobalz.Arc.ArcRec.dt.Rows(i).Item("vid").ToString)
