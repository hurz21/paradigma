    ...
    Sub flstOhnePolygonSuchen()
     ...
     "User Id=" & vorgangrec.mydb.username & ";Password=" & vorgangrec.mydb.password & ";")
        ...
        For Each drr As DataRow In vorgangrec.dt.Rows
            ...
            Console.WriteLine(i & " " & anzahlOhnePolygon & " (" & anzahl & "=")
            ' Debug.Print("aktuell vid=" & vid)
            'ereignis holen
            ereignisRec.mydb.SQL = " select * from raumbezug2geopolygon " &
                                   "  where raumbezugsid=" & rid & ""
            hinweis = ereignisRec.getDataDT()
            If ereignisRec.dt.Rows.Count < 1 Then
                anzahlOhnePolygon += 1
                Dim korrigieren As Boolean = True
                If korrigieren Then
                    sekid = CStr(drr.Item("sekid"))
                    'fs holen
                    If CInt(sekid) < 1 Then Continue For

                    beteiligterec.mydb.SQL = "select * from paraflurstueck where id=" & sekid
                    hinweis = beteiligterec.getDataDT()
                    Try
                        gemcode = CStr(fieldvalue(beteiligterec.dt.Rows(0).Item("gemcode")))
                    Catch ex As Exception
                        gemcode = "0"
                        Continue For
                    End Try

                    fs = CStr(fieldvalue(beteiligterec.dt.Rows(0).Item("fs")))

                    If Not String.IsNullOrEmpty(fs) Then
                        'vid holen
                        beteiligterec.mydb.SQL = "select * from pf_sekid2vid where sekid=" & sekid
                        hinweis = beteiligterec.getDataDT()
                        vid = CInt(beteiligterec.dt.Rows(0).Item("vorgangsid"))

                        Dim erfolg As Boolean = getPolygonFromShapeFile(gemcode, fs, vid)
                        If erfolg Then ifixed += 1
                    Else
                        ohneFS += 1
                    End If
                End If
