...
Public Class clsBeteiligteBUSI
    ...
    Public Shared Function viaKopplung_VorgangID_zu_BeteiligteID(ByVal vorgangsid As String, ByVal Ziel_tidb As IDB_grundfunktionen) As Boolean '  myGlobalz.sitzung.tempREC
        ...
        Ziel_tidb.mydb.SQL = _
                                        "SELECT * FROM " & Ziel_tidb.mydb.Tabelle & _
    ...
    Public Shared Sub holeBeteiligte()
        ...
        'myGlobalz.sitzung.tempREC.mydb.SQL = _
        '"select CONCAT(Anrede,' ', Namenszusatz,' ',vorname,' ',nachname," + _
        '"' ',cast(PLZ as char),' ',Gemeindename,' ',strasse,' ',hausnr,' Kassenkonto: ',Kassenkonto) as abstract " + _
        '" from " & _
        ' myGlobalz.sitzung.tempREC.mydb.Tabelle & _
        ' " where vorgangsid=" & myGlobalz.sitzung.VorgangsID & _
        ' " order by	abstract asc"

        myGlobalz.sitzung.tempREC.mydb.SQL = _
            "select Anrede||' '||Namenszusatz||' '||vorname||' '||nachName||' '||' '||PLZ||' '||Gemeindename||' '||strasse||' '||hausnr||' Kassenkonto: '||Kassenkonto  as abstract" &
               " from " & _
         myGlobalz.sitzung.tempREC.mydb.Tabelle & _
         " where vorgangsid=" & myGlobalz.sitzung.aktVorgangsID & _
         " order by	abstract  asc"
    ...
    Public Shared Function initBeteiligteDatatable(ByVal vid As Integer, ByVal idb As IDB_grundfunktionen) As Boolean   ' myGlobalz.sitzung.beteiligteREC
        If clsBeteiligteBUSI.erzeugeBeteiligtenListezuVorgang("select * from beteiligte where vorgangsid=" & vid, idb) Then                'myGlobalz.sitzung.personenRec
            Return True
        Else
    ...
    Public Shared Function BeteiligteKopplungVorgang_alledb(ByVal zielvid%, ByVal aktRID%, ByVal status As Integer) As Integer
        ...
        If (myGlobalz.beteiligte_MYDB.dbtyp = "oracle") Then
            Dim zzz As New clsBeteiligteDBCRUD_ORACLE(LIBoracle.clsDBspecOracle.getConnection(myGlobalz.sitzung.VorgangREC.mydb))
