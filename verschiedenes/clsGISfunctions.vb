Public Class clsGISfunctions
    ...
    Public Overloads Sub mittelpunktsAufruf(ByVal bbox As clsRange, layer As String, ByVal domain As String)
        ...
        End If
        '  Dim myguid As String = Guid.NewGuid().ToString
        Dim http As String = "http://" & domain & "/cgi-bin/suchdb.cgi?modus=42" & _
         "&rechts=" & CInt(aktp.X) & _
         "&hoch=" & CInt(aktp.Y) & _
         "&abstand=" & abstand & _
         "&username=" & myGlobalz.sitzung.aktBearbeiter.username & _
    ...
    Public Overloads Sub mittelpunktsAufruf(ByVal aktp As myPoint, ByVal domain As String)
        If Not coordIstOk(aktp) Then
            MessageBox.Show("Leider ist die Koordinatenangabe unbrauchbar! " & vbCrLf & aktp.toString())
            Exit Sub
        End If
        ...
        Dim http$ = "http://" & domain & "/cgi-bin/suchdb.cgi?modus=42" & _
         "&rechts=" & CInt(aktp.X) & _
         "&hoch=" & CInt(aktp.Y) & _
         "&abstand=1000" & _
         "&username=" & myGlobalz.sitzung.aktBearbeiter.username & _
    ...
    Public Sub GoogleMapsAufruf_Mittelpunkt(ByVal bbox As clsRange)
        ...
        result = modKoordTrans.Mykoordtransform(myGlobalz.ProxyString, aufruf)
