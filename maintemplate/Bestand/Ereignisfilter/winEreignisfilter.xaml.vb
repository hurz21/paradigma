...
Public Class winEreignisfilter
    ...
    Private Sub btnstartSucheEreignisse_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        If String.IsNullOrEmpty(tbFilterEreignisse.Text) Then
            tbFilterEreignisse.Text = ""
        Else
            tbFilterEreignisse.Text = tbFilterEreignisse.Text.ToLower.Trim
            tbFilterEreignisse.Text = LIBgemeinsames.clsString.normalize_Filename(tbFilterEreignisse.Text, " ")
            tbFilterEreignisse.Text = tbFilterEreignisse.Text.Replace("-", " ").Replace("%", " ")
        End If
        Dim art As String
        ...
        Else
            ...
            dgEreignisse.DataContext = myGlobalz.sitzung.VorgangREC.dt
    ...
    Private Sub btnSgtree2_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        ...
        CLstart.myc.AZauswahl.az.sachgebiet.Header = sgt.publicsgHeader
            ...
            l("btnSgtree2_Click---------------------- ende")
        Catch ex As Exception
            l("Fehler in btnSgtree2_Click: " & ex.ToString())
        End Try
        e.Handled = True
    End Sub
    ...
    Function comboBearbeiterInit() As Boolean
        Try
            Dim sql$ = "select LOWER(INITIAL_) as ""INITIALE"",NACHNAME from " & CLstart.myViewsNTabs.tabBearbeiter & "  where aktiv=1 order by abteilung, NACHNAME asc"
