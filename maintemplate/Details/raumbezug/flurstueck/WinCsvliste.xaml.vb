Public Class WinCsvliste
    ...
    Private Sub btnEinlesen_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        ...
        messresult = MessageBox.Show("Die folgende Aktion kann länger dauern. In dieser Zeit ist die Anwendung blockiert. " & Environment.NewLine &
                                     "Das liegt daran, daß die Geometrie der Flurstücke aus den GIS-Daten geholt werden muss." & Environment.NewLine &
                                     "Es empfiehlt sich sowas während der Mittagspause laufen zu lassen." & Environment.NewLine &
                                     "Möchten Sie jetzt " & Environment.NewLine &
                                     " - weitermachen  = Taste 'Ja'" & Environment.NewLine &
                                     " - abbrechen     = Taste 'Nein'" & Environment.NewLine,
    ...
    Private Function csv_zeilen_zuFST_verarbeiten(ByVal rec As String(), ByVal radius As Double, ByVal titel As String, ByVal fehlerprotokoll As IO.FileInfo, ByVal startspalte As Integer) As String
        ...
        Catch ex As Exception
            nachricht("Fehler in csv_zeilen_zuFST_verarbeiten: " & ex.ToString)
    ...
    Private Sub WinCsvliste_Loaded(ByVal sender As Object, ByVal e As System.Windows.RoutedEventArgs) Handles Me.Loaded
        tbINfo.Text = "Was ist der Hintergrund ?" & Environment.NewLine
        tbINfo.Text &= "Man kann ein Projektgebiet als Polygon im GIS abgrenzen." & Environment.NewLine
        tbINfo.Text &= "Hierbei sind die Kollegen/innen vom GIS hilf gern behilflich!" & Environment.NewLine
        tbINfo.Text &= "Dieser Bereich wird nun mit der Flurkarte 'verschnitten'!" & Environment.NewLine
        tbINfo.Text &= "Man erhält dadurch eine Flurstücksliste. Daraus kann man eine CSV-Datei machen." & Environment.NewLine
        tbINfo.Text &= "Diese CSV-Datei läßt sich hier nun in Paradigma einlesen." & Environment.NewLine
        tbINfo.Text &= "Es wird für jedes Flurstück ein Raumbezug erzeugt." & Environment.NewLine
    ...
    Private Sub btnDateistruktur_Click(ByVal sender As System.Object, ByVal e As System.Windows.RoutedEventArgs)
        Dim text As String = ""
        text = "Die 1.Zeile enthält die Spaltennamen - KEINE Daten!" & Environment.NewLine &
            "Die ersten 3 Spalten sind leer (sofern Startspalte=4)" & Environment.NewLine &
            "4.Spalte: Gemarkungscode (z.B. 743 für Langen)" & Environment.NewLine &
            "5.Spalte: Flurnummer" & Environment.NewLine &
            "6.Spalte: Zaehler" & Environment.NewLine &
            "7.Spalte: Nenner " & Environment.NewLine &
            "" & Environment.NewLine &
            "Weitere Spalten werden ignoriert." & Environment.NewLine &
            "Trennzeichen ist das Semikolon ;" & Environment.NewLine &
            "Das Format kann einfach aus Excel exportiert werden: CSV Dos" & Environment.NewLine &
            "" & Environment.NewLine &
            "" & Environment.NewLine &
            "" & Environment.NewLine &
            "" & Environment.NewLine
        MessageBox.Show(text, "Dateistruktur für Flurstücksimport")
        e.Handled = True
    End Sub
