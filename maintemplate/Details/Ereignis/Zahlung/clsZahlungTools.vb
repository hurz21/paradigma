...
Namespace ZahlungToolsNs
    ...
    Public Class ZahlungNeu_alleDB
        Public Shared Function execute() As Integer
            ...
            '    Dim zzz As New clsZAHLUNGDB_Oracle(clsDBspecOracle.getConnection(myGlobalz.zahlung_MYDB))
            '    parafotoID = zzz.Neu_speichern_zahlung()
            '        zzz.dispose
            'End If
            'Return parafotoID%
    ...
    Public Class leseZahlung
        Shared Sub execute(ByVal ereignisid%, ByVal vorgangsID%)
            'Dim sql$
            'sql$ = String.Format("select * from " & CLstart.myViewsNTabs.tabZahlungen & "  where ereignisid={0} and vorgangsid={1}", ereignisid, vorgangsID)
            'If myGlobalz.zahlung_MYDB.dbtyp = "mysql" Then
            '    Dim zzz As New clsZAHLUNGDB_Mysql(clsDBspecMYSQL.getConnection(myGlobalz.zahlung_MYDB))
    ...
    Public Class alleZahlungen
        Public Shared Sub execute()
            ...
            'Dim sql$ = "select  * fromvorgang order by sachgebietnr"
            myGlobalz.sitzung.tempREC.dt = New DataTable
            Dim anzahl As Integer = VSTTools.selectFromParadigmaTabelle_alleDB.exe(sql, myGlobalz.sitzung.tempREC.dt)
            'If myGlobalz.zahlung_MYDB.dbtyp = "mysql" Then
            '    Dim zzz As New clsZAHLUNGDB_Mysql(clsDBspecMYSQL.getConnection(myGlobalz.zahlung_MYDB))
            '    zzz.getDT_zahlung(sql)
            '    zzz.dispose
