	...
	Public Shared Function gemeinsames_element(ByVal layers$(), ByVal ug$()) As Boolean
		...
		Catch ex As Exception
			Dim FehlerHinweis$ = ", Fehler: " & vbCrLf + _
			 ...
			 ex.Source & " "
			'mylog.log("ERROR /fehler in gemeinsames_element" & FehlerHinweis$)
			'mylog.log(FehlerHinweis)
			Return False
		End Try
	End Function
...
#End Region
	...
	Public Shared Function Normalize_Slashes(ByVal dateifullname$) As String
		Try
			Dim dateifullnamenorm$ = dateifullname$.Replace("/", "\")
			If dateifullnamenorm$.Contains(":") Then dateifullnamenorm$ = dateifullnamenorm$.Replace("\\", "\")
			Return dateifullnamenorm$
		Catch ex As Exception
			Return Nothing
		End Try
	End Function
