...
#Region "RB_adresse"
    Public Shared Sub hole_AdressKoordinatenExtracted(ByVal halo_id%)
        ...
        myGlobalz.sitzung.haloREC.mydb.SQL = "select rechts,hoch,fs from " & _
    ...
    Public Shared Sub DBholeHausnrDT()
        myGlobalz.sitzung.haloREC.mydb.Schema = "halosort"
        myGlobalz.sitzung.haloREC.mydb.Tabelle = "halofs"
        myGlobalz.sitzung.haloREC.mydb.SQL = String.Format("select distinct id ,cast(concat(hausnr,zusatz) as CHAR) as hausnrkombi from {0} where gemeindenr = {1} and strcode = {2} order by  hausnr,zusatz", _
    ...
    Public Shared Sub holeStrasseDT()
        myGlobalz.sitzung.haloREC.mydb.Schema = "halosort"
        myGlobalz.sitzung.haloREC.mydb.Tabelle = "halofs"
        myGlobalz.sitzung.haloREC.mydb.SQL = "select distinct strcode ,sname from " & _
         myGlobalz.sitzung.haloREC.mydb.Tabelle & _
         " where gemeindenr = " & myGlobalz.sitzung.aktADR.Gisadresse.gemeindeNrBig() & _
         " order by	sname asc"
        nachricht(myGlobalz.sitzung.haloREC.getDataDT())
...
#End Region
    ...
    Public Shared Sub hole_FSTKoordinatenExtracted(ByVal FS$)
        myGlobalz.sitzung.fstREC.mydb.SQL = "select rechts,hoch,fs from " & _
                                myGlobalz.sitzung.fstREC.mydb.Tabelle & _
                                " where FS = '" & FS$ & "'"
        nachricht(myGlobalz.sitzung.fstREC.getDataDT())
    End Sub
...
#End Region
    ...
    'Public Shared Function initKontaktdaten4personDatatable(ByVal personenID As Integer) As Boolean
    '    'koppelungstabelle abfragen> adressid-liste
    '    If KontaktIDlisteVonPersonErstellen(personenID%) Then
    '        myGlobalz.sitzung.kontaktdatenDT = myGlobalz.sitzung.tempREC.dt.Copy
    '        myGlobalz.sitzung.tempREC.mydb.Tabelle = "Kontaktdaten"
    '        Dim SQL$ = clsDBtools.UNION_SQL_erzeugenInn(myGlobalz.sitzung.kontaktdatenDT, myGlobalz.sitzung.tempREC.mydb.Tabelle, 2, "kontaktid")
    '        My.Log.WriteEntry("UNION: " & SQL)
    ...
    'Public Shared Function Koppelung_Person_Kontakt(ByVal personenID%, ByVal kontaktID as integer) as  Integer
    '    Dim anzahlTreffer&
    '    Dim newid& = -1
    '    Try
    ...
    Public Shared Function Entkoppelung_Person_Kontakt(ByVal personenID%, ByVal kontaktID As Integer) As Integer
        Dim anzahlTreffer&
        Dim newid& = -1
        Try
            myGlobalz.sitzung.tempREC = CType(myGlobalz.sitzung.VorgangREC, clsDBspecMYSQL)
            ...
            myGlobalz.sitzung.tempREC.mydb.SQL = _
             ...
             "  where kontaktID=" & kontaktID% & _
             " and PersonenID=" & personenID%
            anzahlTreffer = myGlobalz.sitzung.tempREC.sqlexecute(newid) ')
            If anzahlTreffer < 1 Then
                nachricht_und_Mbox("Problem beim Abspeichernx:" & myGlobalz.sitzung.tempREC.mydb.SQL)
                Return -1
            Else
                Return CInt(anzahlTreffer)
            End If
        Catch ex As Exception
            nachricht_und_Mbox("Entkoppelung_Person_Kontakt: " & vbCrLf & ex.ToString)
            Return -2
        End Try
    End Function
