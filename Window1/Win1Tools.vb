...
Namespace Paradigma_start
    ...
    Public Class Win1Tools
        ...
        Shared Sub initgisuser(ByRef GISuser As String)
            ...
            'If myGlobalz.halo_MYDB.dbtyp = "mysql" Then
            '    Dim wc As New LIBwebgiscontrol.clsWebgiscontrol_Mysql
            '    myGlobalz.sitzung.Bearbeiter.GISPassword = wc.holePasswordFuerGisUser(GISuser, myGlobalz.sitzung.haloREC)
            '    ' myGlobalz.sitzung.Bearbeiter.GISPassword = LIBwebgiscontrol.clsWebgiscontrol_Mysql.holePasswordFuerGisUser(GISuser$, myGlobalz.sitzung.haloREC)
            'End If
            myGlobalz.sitzung.aktBearbeiter.GISPassword = clsMiniMapTools.holePasswordFuerGisUser_dballe(myGlobalz.sitzung.aktBearbeiter.username, myGlobalz.sitzung.haloREC)
            'Todo 
            'If myGlobalz.halo_MYDB.dbtyp = "oracle" Then
            '    Dim wc As New LIBwebgiscontrol.clsWebgiscontrol_oracle
            '    myGlobalz.sitzung.Bearbeiter.GISPassword = wc.holePasswordFuerGisUser(GISuser, myGlobalz.sitzung.haloREC)
            'End If
